...
/**
 * 
 * GeometryImpl is the root class of the geometric object taxonomy and supports methods common to all geographically referenced geometric objects.
 * GeometryImpl instances are sets of direct positions in a particular coordinate reference system. A GeometryImpl can be regarded as an infinite set
 * of points that satisfies the set operation interfaces for a set of direct positions, TransfiniteSet&lt;DirectPosition&gt;. Since an infinite
 * collection class cannot be implemented directly, a boolean test for inclusion is provided by this class.
 * 
 * NOTE As a type, GeometryImpl does not have a well-defined default state or value representation as a data type. Instantiated subclasses of
 * GeometryImpl will.
 * 
 * 
 *
...
public abstract class GeometryImpl implements Geometry, Serializable {
    ...
    /**
     * Computes the distance between this and another geometry. We have to implement the logic of dealing with multiprimtive geometries separately.
     * 
     * gdavis - This method should really be broken out into 1 of 2 things: 1. an operator class that figures out the geometry type and does the
     * operation based on that, or 2. a double dispatch command pattern system that returns a command to perform based on the geometry type and the
     * command of "distance". Currently this implementation works for our needs, but we should consider re-designing it with one of the above
    ...
    public final double distance(Geometry geometry) {
        ...
        if(g0Dim == 3 && g1Dim == 3) {
            return Geometry3DOperation.distance(this, geometry);
        }
        /* */
        ...
        if (geometry instanceof MultiPrimitive) {
            ...
            if (this instanceof PointImpl) {
                ...
                lines1 = ((RingImplUnsafe) ((SurfaceImpl) this).getBoundary().getExterior())
                        .asLineString().asLineSegments();
            }
            ...
            if (geometry instanceof PointImpl) {
                ...
                lines2 = ((RingImplUnsafe) ((SurfaceImpl) geometry).getBoundary().getExterior())
                        .asLineString().asLineSegments();
            }
            ...
            if (point1 != null && point2 != null) {
                ...
                if (point2 != null) {
                    ...
                    for (int i = 0; i < lines1.size(); i++) {
                        Coordinate c1 = new Coordinate(point2.getRepresentativePoint()
                                .getCoordinate());
                        Coordinate cA = new Coordinate(lines1.get(i).getStartPoint()
                                .getCoordinate());
                    ...
                    for (int i = 0; i < lines1.size(); i++) {
                        for (int y = 0; y < lines2.size(); y++) {
                            Coordinate A = new Coordinate(lines1.get(i).getStartPoint()
                                    .getCoordinate());
                            Coordinate B = new Coordinate(lines1.get(i).getEndPoint()
                                    .getCoordinate());
                            Coordinate C = new Coordinate(lines2.get(y).getStartPoint()
                                    .getCoordinate());
                            Coordinate D = new Coordinate(lines2.get(y).getEndPoint()
                                    .getCoordinate());
                ...
                if (point1 != null) {
                    ...
                    for (int i = 0; i < lines2.size(); i++) {
                        Coordinate c1 = new Coordinate(point1.getRepresentativePoint()
                                .getCoordinate());
                        Coordinate cA = new Coordinate(lines2.get(i).getStartPoint()
                                .getCoordinate());
    ...
    public boolean within(TransfiniteSet pointSet) {
        ...
        if(d1 == 3 && d2 == 3) {
            return Geometry3DOperation.contains(geom, this);
        }
        /* */
    ...
    /**
     * This operator tests, whether an object overlaps with this object. That is that a part of the object lies within this object and another part
     * lies without this object, e.g. the other object intersects with the interior, boundary and exterior of this object
     * 
     * @param pointSet The other object
     * 
    ...
    public Complex getClosure() {
        if (this instanceof ComplexImpl) {
            ...
            Assert.isTrue(false,
                    "The closure operation is not implemented for this geometry object");
        return null;
    ...
    /**
     * Use this function to cast Geometry instances to a GeometryImpl instance. In that way we can control the illegal injection of other instances at
     * a central point.
     * 
    ...
    /**
     * Use this function to cast TransfiniteSet instances to a GeometryImpl instance. In that way we can control the illegal injection of other
     * instances at a central point.
     * 
     * @param tf
     * @return
     */
