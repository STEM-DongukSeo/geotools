...
public class GeometryFactoryImpl implements Factory, GeometryFactory {
     ...
     * <p>
     * FactoryRegistry will check to make sure that there are no duplicates created (so there will be only a "single" PositionFactory created with
     * this configuration).
     * </p>
     */
     ...
     * (non-Javadoc)
     * 
     * @see org.opengis.geometry.coordinate.Factory#createEnvelope(org.opengis.geometry.coordinate.DirectPosition,
    ...
    public Envelope createEnvelope(DirectPosition lowerCorner, DirectPosition upperCorner)
        ...
        // Test ok
        return new EnvelopeImpl((DirectPosition) positionFactory.createPosition(lowerCorner),
     ...
     * (non-Javadoc)
     * 
     * @see org.opengis.geometry.coordinate.Factory#createLineSegment(org.opengis.geometry.coordinate.Position,
     ...
     */
    public Collection<DirectPosition> createDirectPositions(Collection<double[]> coordList,
    ...
    /**
     * Creates a line segment between fromPosition and toPosition. The coordinate dimension of the segment is fromPosition.length, fromPosition.length
     * and toPoint.length must be equal and greater then 1
     * 
     ...
     */
    public LineSegmentImpl createLineSegment(double[] fromPosition, double[] toPosition,
            double startPar) {
        ...
        return createLineSegment(this.createPosition(fromPosition),
                this.createPosition(toPosition), startPar);
    }
     ...
     */
    public Collection<LineStringImpl> createLineStrings(Collection<List<double[]>> coordLists,
     ...
     * @return Triangle
     */
    public TriangleImpl createTriangle(TriangulatedSurface ts, DirectPosition p1,
        ...
        return new TriangleImpl(triangleBoundary, ts, new PositionImpl(p1), new PositionImpl(p2),
                new PositionImpl(p3));
    }
    ...
    public ArrayList<Triangle> createTriangles(ArrayList<double[][]> triangles) {
        ...
        for (double[][] triangle : triangles) {
            ...
            result.add(this.createTriangle(null, createDirectPosition(coord0),
                    createDirectPosition(coord1), createDirectPosition(coord2)));
        }
        return result;
    }
    ...
    public ArcStringByBulge createArcStringByBulge(List<Position> points, double[] bulges,
            List<double[]> normals) throws MismatchedReferenceSystemException,
    ...
    public BSplineCurve createBSplineCurve(int degree, PointArray points, List<Knot> knots,
    ...
    public BSplineSurface createBSplineSurface(List<PointArray> points, int[] degree,
            List<Knot>[] knots, KnotType knotSpec) throws MismatchedReferenceSystemException,
            MismatchedDimensionException {
        // LATER:
        // TO DO semantic SJ, JR
        // TO DO implementation
    ...
    public PolyhedralSurface createPolyhedralSurface(List<Polygon> tiles)
        ...
        // TO DO test
        // TO DO documentation
        return new PolyhedralSurfaceImpl(getCoordinateReferenceSystem(), tiles);

    }

    public TriangulatedSurface createTriangulatedSurface(List<Polygon> tiles)
            throws MismatchedReferenceSystemException, MismatchedDimensionException {
        return new TriangulatedSurfaceImpl(getCoordinateReferenceSystem(), tiles);

    }

    public Tin createTin(Set<Position> post, Set<LineString> stopLines, Set<LineString> breakLines,
