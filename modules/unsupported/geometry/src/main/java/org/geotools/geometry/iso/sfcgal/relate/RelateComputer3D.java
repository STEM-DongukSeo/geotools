...
public class RelateComputer3D {
    ...
    public IntersectionMatrix3D computeIM3D() {
        ...
        /*
        if (!intersectionCC.isEmpty() && !intersectionBC.isEmpty()) {
            SFGeometry difference = SFAlgorithm.difference3D(intersectionCC, intersectionBC);
            // System.out.println("equals : " + intersectionCC.equals(intersectionBC));
            // System.out.println("differene : " + difference.asText(0));
            if (intersectionCC.equals(intersectionBC) || difference.isEmpty()) {
                // II = F, BI = T
                tIM.set(Location.INTERIOR, Location.INTERIOR, Dimension.FALSE);
            ...
            difference = SFAlgorithm.difference3D(intersectionCB, intersectionBB);
            if (intersectionCB.equals(intersectionBB) || difference.isEmpty()) {
                // IB = F
                // BB = T
                tIM.set(Location.INTERIOR, Location.BOUNDARY, Dimension.FALSE);
            ...
            if (!intersectionCC.isEmpty() && intersectionBC.isEmpty()) {
                ...
                tIM.set(Location.BOUNDARY, Location.INTERIOR, Dimension.FALSE);
                // tIM.set(Location.BOUNDARY, Location.BOUNDARY, Dimension.FALSE);
            } else if (intersectionCC.isEmpty()) {
                // II = F, IB = F, BI = F, BB = F
                tIM.set(Location.INTERIOR, Location.INTERIOR, Dimension.FALSE);
                tIM.set(Location.INTERIOR, Location.BOUNDARY, Dimension.FALSE);
                tIM.set(Location.BOUNDARY, Location.INTERIOR, Dimension.FALSE);
            ...
            if (!intersectionCB.isEmpty() && intersectionBB.isEmpty()) {
                // IB = T
                // BB = F
                tIM.set(Location.INTERIOR, Location.BOUNDARY, intersectionCB.dimension());
                tIM.set(Location.BOUNDARY, Location.BOUNDARY, Dimension.FALSE);
            } else if (intersectionCB.isEmpty()) {
                // II = F, IB = F, BI = F, BB = F
                tIM.set(Location.INTERIOR, Location.INTERIOR, Dimension.FALSE);
                tIM.set(Location.INTERIOR, Location.BOUNDARY, Dimension.FALSE);
                tIM.set(Location.BOUNDARY, Location.INTERIOR, Dimension.FALSE);
        ...
        for (int i = 0; i < lineString.numPoints() - 1; i++) {
            ...
            case SFCGALConvertor.SFCGAL_MULTILINESTRING_ID:
                for (int j = 0; j < intersection.numGeometries(); j++) {
                    merge(points, intersection.geometryN(j));
                }
                break;
            default:
    ...
    public void merge(ArrayList<SFPoint> points, SFGeometry geometry) {
        ...
        if (geometry.geometryTypeId() == SFCGALConvertor.SFCGAL_POINT_ID) {
            ...
            for (int i = 0; i < line.numPoints(); i++) {
                if (!isContain(points, line.pointN(i))) {
                    points.add(line.pointN(i));
                }
            }
        }
    }
