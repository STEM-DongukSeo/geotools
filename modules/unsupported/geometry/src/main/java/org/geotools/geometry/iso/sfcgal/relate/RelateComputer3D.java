...
public class RelateComputer3D {
    ...
    public IntersectionMatrix3D computeIM3D() {
        ...
        /*
         * System.out.println("intersectionCC : " + intersectionCC.asText(1)); // SF_Geometry difference = SF_Algorithm.difference3D(intersectionCC,
         * intersectionBC); // System.out.println("difference : " + difference.asText(1)); System.out.println("intersectionBC : " +
         * intersectionBC.asText(1)); System.out.println("intersectionCB : " + intersectionCB.asText(1)); System.out.println("intersectionBB : " +
         * intersectionBB.asText(1));
         */
         ...
         * difference.asText(0)); if (intersectionCC.equals(intersectionBC) || difference.isEmpty()) { // II = F, BI = T tIM.set(Location.INTERIOR,
         * Location.INTERIOR, Dimension.FALSE); tIM.set(Location.BOUNDARY, Location.INTERIOR, intersectionBC.dimension()); } else { // II = T // BI =
         * T tIM.set(Location.INTERIOR, Location.INTERIOR, intersectionCC.dimension()); tIM.set(Location.BOUNDARY, Location.INTERIOR,
         * intersectionBC.dimension()); }
         * 
         * difference = SFAlgorithm.difference3D(intersectionCB, intersectionBB); if (intersectionCB.equals(intersectionBB) || difference.isEmpty()) {
         * // IB = F // BB = T tIM.set(Location.INTERIOR, Location.BOUNDARY, Dimension.FALSE); tIM.set(Location.BOUNDARY, Location.BOUNDARY,
         * intersectionBB.dimension()); } else { // IB = T // BB = T tIM.set(Location.INTERIOR, Location.BOUNDARY, intersectionCB.dimension());
         * tIM.set(Location.BOUNDARY, Location.BOUNDARY, intersectionBB.dimension()); } } else { if (!intersectionCC.isEmpty() &&
         * intersectionBC.isEmpty()) { // II = T, IB = T, BI = F, BB = F // II = T, IB = ?, BI = F, BB = ? tIM.set(Location.INTERIOR,
         * Location.INTERIOR, intersectionCC.dimension()); // tIM.set(Location.INTERIOR, Location.BOUNDARY, intersectionCB.dimension());
         * tIM.set(Location.BOUNDARY, Location.INTERIOR, Dimension.FALSE); // tIM.set(Location.BOUNDARY, Location.BOUNDARY, Dimension.FALSE); } else
         * if (intersectionCC.isEmpty()) { // II = F, IB = F, BI = F, BB = F tIM.set(Location.INTERIOR, Location.INTERIOR, Dimension.FALSE);
         * tIM.set(Location.INTERIOR, Location.BOUNDARY, Dimension.FALSE); tIM.set(Location.BOUNDARY, Location.INTERIOR, Dimension.FALSE);
         * tIM.set(Location.BOUNDARY, Location.BOUNDARY, Dimension.FALSE); }
         * 
         * if (!intersectionCB.isEmpty() && intersectionBB.isEmpty()) { // IB = T // BB = F tIM.set(Location.INTERIOR, Location.BOUNDARY,
         * intersectionCB.dimension()); tIM.set(Location.BOUNDARY, Location.BOUNDARY, Dimension.FALSE); } else if (intersectionCB.isEmpty()) { // II =
         * F, IB = F, BI = F, BB = F tIM.set(Location.INTERIOR, Location.INTERIOR, Dimension.FALSE); tIM.set(Location.INTERIOR, Location.BOUNDARY,
         * Dimension.FALSE); tIM.set(Location.BOUNDARY, Location.INTERIOR, Dimension.FALSE); tIM.set(Location.BOUNDARY, Location.BOUNDARY,
         * Dimension.FALSE); } }
         */
    ...
    public SFGeometry modifyLineString(SFGeometry line, SFGeometry other) {
        ...
        for (int i = 0; i < lineString.numPoints() - 1; i++) {
            ...
            SFLineString tempLine = new SFLineString(lineString.pointN(i), lineString.pointN(i + 1));
            ...
            case SFCGALConvertor.SFCGAL_MULTILINESTRING_ID:
                ...
                for (int j = 0; j < intersection.numGeometries(); j++) {
                    ...
                    for (int k = 0; k < orderedGeometry.size(); k++) {
                        double d2 = SFAlgorithm.distance3D(lineString.pointN(i),
                                orderedGeometry.get(k));

                        if (d1 < d2) {
                            orderedGeometry.add(k, intersection.geometryN(j));
                            isInserted = true;
                            break;
                        }
                    }
    ...
    public void merge(ArrayList<SFPoint> points, SFGeometry geometry) {
        ...
        if (geometry.geometryTypeId() == SFCGALConvertor.SFCGAL_POINT_ID) {
            ...
            for (int i = 0; i < line.numPoints(); i++) {
                double d1 = SFAlgorithm.distance3D(points.get(points.size() - 1), line.pointN(i));
                boolean isInserted = false;
                for (int j = 0; j < orderedPoints.size(); j++) {
                    double d2 = SFAlgorithm.distance3D(points.get(points.size() - 1),
                            orderedPoints.get(j));
